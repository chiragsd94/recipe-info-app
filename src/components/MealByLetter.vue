<template>
    <div>
      ...
      <p class="text-gray-500">Loading...</p>
        ...
        <div v-else>
            <div v-if="cabcLenght" class="text-center text-xl text-slate-500 m-2 p-2">
                Sorry there is no meal from {{ props.letter }}
            </div>
        <div v-else class="flex flex-wrap"> 
            <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 p-2" 
            v-for="meal in cabc" :key="meal.idMeal">
                {{ meal.strMeal }}
...
const cabc = computed(()=> {return abcd.value.filter((a)=> a.strMeal[0] === l || a.strMeal[0] === l.toUpperCase())})
const cabcLenght = computed(()=>{return cabc.value.length === 0})
onMounted(async ()=>{
    await store.search(l);
    isLoading.value=false
    abcd.value = mealsData;
})
